(async()=>{
    ...
    s4d.client.on('ready', async () => {
      s4d.client.user.setPresence({status: "online",activities:[{name:(String((s4d.client.guilds.cache.get('932651844344373278')).memberCount) + ' members in S4D'),type:"WATCHING"}]});
    ...
    s4d.client.on('messageCreate', async (s4dmessage) => {
      if (((s4dmessage.channel).type) == 'GUILD_NEWS') {
        ...
        s4dmessage.reply({content:String((['Hi ',(s4dmessage.member.user).username,', i have only slash commands!'].join(''))), allowedMentions: {
                repliedUser: true
            }});
        ...
        if ((String((s4dmessage.content)).includes(String('thx'))) || (String((s4dmessage.content)).includes(String('thanks'))) || (String((s4dmessage.content)).includes(String('thank you')))) {
          s4dmessage.channel.send({content:String('use `/review` in <#932651844973502473> to rate your helper(s)!')});
        }
      }
    ...
    s4d.client.on('interactionCreate', async (interaction) => {
            let member = interaction.guild.members.cache.get(interaction.member.user.id)
              ...
              switch ((interaction.options.getSubcommand())) {
                ...
                await interaction.reply({ content: 'shuting down...', ephemeral: true, components: [] });
                await delay(Number(2)*1000);
                s4d.client.destroy();
              } else {
                await interaction.reply({ content: '<a:youtried:939133664887988264>', ephemeral: true, components: [] });
              }
            ...
            break;case 'info':
                ...
                await interaction.reply({embeds: [embed1], ephemeral: false, components: [] });
        ...
        case 'help':
            ...
            'autoresponse (show/set/remove) - add, removes keywords or see all of them ','','',''].join(''))));
            await interaction.reply({embeds: [embed1], ephemeral: false, components: [] });
        ...
        case 'autoresponse':
            ...
            switch ((interaction.options.getSubcommand())) {
              ...
              break;case 'add':
                ...
                await interaction.reply({ content: (['added `',interaction.options.getString('keyword'),'` to database"'].join('')), ephemeral: true, components: [] });
                (((interaction.guild).members.cache.get('513095506914705418') || await (interaction.guild).members.fetch('513095506914705418'))).send({content:String((['added keyword `',(interaction.options.getString('keyword')).toLowerCase(),'` with name ',interaction.options.getString('name'),' and description ',String((interaction.options.getString('description'))).replaceAll('>', String('\n')),'\n',' and image ',interaction.options.getString('image'),interaction.member.user].join('')))});
    
              break;case 'remove':
                    if (database2.has(String(((interaction.options.getString('keyword')).toLowerCase())))) {
                  (((interaction.guild).members.cache.get('513095506914705418') || await (interaction.guild).members.fetch('513095506914705418'))).send({content:String((['removed keyword `',(interaction.options.getString('keyword')).toLowerCase(),'` with name ',database2.get(String(((interaction.options.getString('keyword')).toLowerCase())))[0],' and description ',database2.get(String(((interaction.options.getString('keyword')).toLowerCase())))[1],' and image ',database2.get(String(((interaction.options.getString('keyword')).toLowerCase()))).slice(-1)[0],'\n',interaction.member.user].join('')))});
                  database2.delete(String((interaction.options.getString('keyword'))));
                  await interaction.reply({ content: (['Keyword `',(interaction.options.getString('keyword')).toLowerCase(),'` removed'].join('')), ephemeral: true, components: [] });
                } else {
                  await interaction.reply({ content: (['Keyword `',interaction.options.getString('keyword'),'` not found'].join('')), ephemeral: true, components: [] });
                }
            ...
            await interaction.reply({ content: 'You dont have access to use this command!', ephemeral: true, components: [] });
          }
        ...
        case 'report':
            ...
            embed.setColor(s4dcolor);
            s4d.client.channels.cache.get('932651845724291084').send({ embeds: [embed] });
    
          await interaction.reply({ content: 'Thanks for your report, please wait until the staff team view it, in meantime take a cookie 🍪!', ephemeral: true, components: [] });
        ...
        case 'helper':
              ...
              let newReview =
              {
                  "rating": (interaction.options.getString('rating')),"comment": (interaction.options.getString('comment')),"timestamp": (new Date().getTime()),
              };
              ...
              if ((reviewers[String(user)]).includes(helper)) {
                await interaction.reply({ content: 'you have already reviewed this helper. ', ephemeral: false, components: [] });
                (interaction.channel).send(String('do you want to edit your review?')).then(() => { (interaction.channel).awaitMessages({filter:(m) => m.author.id === (interaction.member).id,  time: (5*60*1000), max: 1 }).then(async (collected) => { s4d.reply = collected.first().content;
                 s4d.message = collected.first();
                   if (!(((s4d.reply) || '').startsWith('y' || ''))) {
                    (interaction.channel).send({content:String('cancelled')});
                    return
                  }
                 ...
                 s4d.reply = null; }).catch(async (e) => { console.error(e);   (interaction.channel).send({content:String('cancelled')});
                 });
                })
                return
              }
            ...
            case 'helper-list':
              ...
              block
                  */
                 await interaction.reply({ content: 'getting helpers', ephemeral: true, components: [] });
              var reviewed = reviews.get(String('reviewed'));
              let pages = [[]]
              ...
              var embed = new Discord.MessageEmbed();
                ...
                embed.setFooter({text: String((['page ',page,'/',pages.length].join(''))), iconURL: String()});
    
              (interaction.channel).send({embeds: [embed],components:[buttons_right]}).then(async m=>{
                ...
                collector.on('collect', async i => {
                  ...
                  be correct
                     ...
                     if (page == 1) {
                    if (pages.length == 1) {
                      await i.update({embeds: [embed],components:[buttons_none]}).then(async m=>{
    
                                  });
                    } else {
                      await i.update({embeds: [embed],components:[buttons_right]}).then(async m=>{
    
                                  });
                    }
                  } else if (page == pages.length) {
                    await i.update({embeds: [embed],components:[buttons_left]}).then(async m=>{
    
                                });
                  } else {
                    await i.update({embeds: [embed],components:[buttons_all]}).then(async m=>{
